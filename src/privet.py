...
class PRIVET:
    ...
    Parameters
    ----------
    train : np.ndarray or torch.Tensor
    ...
    def __init__(
        ...
        self.train = train if isinstance(train, torch.Tensor) else torch.tensor(train)
        self.device = device
        ...
        sorted_distances, sorted_indices = sorting(dist_tr_tr, idx_tr_tr)

        self.p_tr_tr_NN_dist = np.array(sorted_distances)
        self.p_tr_tr_NN_idx = np.array(sorted_indices)
    ...
    def _compute_one_nn(self, src, dst, k=1, distance="standard_euclidean", chunk_size=128):
        """
        Wrapper around gpu_nearest_neighbors to return distances and indices.

        src : torch.Tensor of shape (N_src, D)
        dst : torch.Tensor of shape (N_dst, D)
        """
        # We assume gpu_nearest_neighbors returns “(distances, indices)” unsorted.

        src = src if isinstance(src, torch.Tensor) else torch.tensor(src)
        dst = dst if dst is None or isinstance(dst, torch.Tensor) else torch.tensor(dst)
    ...
    def _fit_extreme_value(self, nn_distances: np.ndarray, start_frac: float, end_frac: float):
        """
    ...
    def compute_scores_syn_to_ref(self, synthetic, test = None, renormalization=None, groundtruth = None):
        """
        synthetic : np.ndarray or torch.Tensor
            The synthetic data to be evaluated.
        test : np.ndarray or torch.Tensor, optional
        ...
        synthetic = synthetic if isinstance(synthetic, torch.Tensor) else torch.tensor(synthetic)
        test = test if test is None or isinstance(test, torch.Tensor) else torch.tensor(test)

        groundtruth_flag = False
        if groundtruth is not None: groundtruth_flag = True

        # 1) Build “synthetic→train” table
        mat_syn_train = self._build_syn_table(
            src=synthetic, dst=self.train,
            include_groundtruth=groundtruth
        )

        # If test is provided, build a second table
        if test is not None:
            mat_syn_test = self._build_syn_table(
                src=synthetic, dst=test,
            ...
            if renormalization is None:
                _lambda = N_tr/N_te
                if _lambda == 1 or self.label_best_fit == "Gumbel": renormalization = 1
                if _lambda > 1 and self.label_best_fit == "Weibull": renormalization = _lambda**(-1/self.param2)
                if _lambda < 1 and self.label_best_fit == "Weibull": renormalization = _lambda**(1/self.param2)
        ...
        train_lookup = {
            int(row[1]): i_rank for i_rank, row in enumerate(mat_syn_train)
        }
        if mat_syn_test is not None:
            test_lookup = {
                int(row[1]): i_rank for i_rank, row in enumerate(mat_syn_test)
            }
        ...
        for i_syn in range(N_syn):
            ...
            store_in_mat[i_syn, self.COL_ONE_MINUS_PI_TRAIN] = one_minus_pi_train #(out_score_alternative_order_stat)
            store_in_mat[i_syn, self.COL_N_OVERFIT_RANK_R] = n_overfit_rank_r #(out_score_aggreg)
            ...
            if mat_syn_test is not None:
                ...
                px_test_rank_r = cdf_func(dist_test_rank_r, self.param1, self.param2).item()
                px_test_prev = cdf_func(dist_test_prev, self.param1, self.param2).item()
                px_test_rank_r_prev = cdf_func(dist_test_rank_r_prev, self.param1, self.param2).item()
                ...
                one_minus_pi_test_rank_r = safe_log10_cdf(rank_s_tr, N_syn, px_test_rank_r)
                ...
                excess_te_rank_r_idx = min(int(1 + np.floor(N_syn * px_test_rank_r_prev)), N_syn - 1)
                ...
                score4 = pi_train - pi_test_rank_r + one_minus_pi_test_rank_r - one_minus_pi_train
                n_privacy_leaks_rank_r = int(N_syn * px_test_rank_r) - int(N_syn * px_train)
                ...
                store_in_mat[i_syn, self.COL_DELTA_PI_RANK_R] = delta_pi_rank_r #(out_score)
                store_in_mat[i_syn, self.COL_DELTA_P_RANK_R] = delta_p_rank_r #(out_score)
                store_in_mat[i_syn, self.COL_BAR_DELTA_PI_RANK_R] = bar_delta_pi_rank_r #(out_score_underfitting)
                store_in_mat[i_syn, self.COL_SCORE4] = score4 #(out_score_alternative)
                store_in_mat[i_syn, self.COL_N_PRIVACY_LEAKS_RANK_R] = n_privacy_leaks_rank_r #(out_score_aggreg)
                ...
                store_in_mat[i_syn, self.COL_ONE_MINUS_PI_TEST_RANK_R] = one_minus_pi_test_rank_r #(out_score_order_stat)
