...
def generate_fake_synth(
    ...
    for s in range(n):
        idx_train = indices[
            s
        ]  # idx of the real (train) sample that is 1-NN of that synthetic
        # Hybridize between synth and train
        snp_mask = np.zeros(L, dtype=bool)
        snp_mask[: int(f_copy * L)] = True
        np.random.shuffle(
            snp_mask
        )  # is it necessary? -> if hamming no, if more complex distance yes
        fake[s] = np.where(snp_mask, train[idx_train], synth[s])
